...
var CURRENT_SETTING_PATH = "SUBLIMESOCKET_PATH:tool/chromeClient/ScalaFilter.txt";

var TSC_SIMPLE_COMPILE_SHELLPATH = "SUBLIMESOCKET_PATH:tool/chromeClients/ScalaClient/tscshell.sh";
...
var SCALAGRADLEC_COMPILATIONTARGETMODE = TARGET_FOLDER;
var SCALAGRADLEC_IDENTIFIED_SENDER_STARTMARK = "scalasaved";
var SCALAGRADLEC_IDENTIFIED_SENDER_ENDMARK = "scalacompilefinished";
...
chrome.browserAction.onClicked.addListener(function(tab){
    if( ssChromeClient_tailing_tab === null && tab.url.indexOf('file://') == 0){
        ...
        // connect.
        _WS.init(tab.url, SCALAGRADLEC_COMPILATIONTARGETMODE);
        _WS.connect();
    }else{
...
var _WS = {
    ...
    onMessage: function (e) {
        if (e.data.indexOf(SCALAGRADLEC_IDENTIFIED_SENDER_STARTMARK) === 0) {
            
            var currentCompileTargetFileName = e.data.replace(SCALAGRADLEC_IDENTIFIED_SENDER_STARTMARK+":","")
        ...
        if (e.data.indexOf(SCALAGRADLEC_IDENTIFIED_SENDER_ENDMARK) === 0) {
            needTail = false;
        }
